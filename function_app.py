...
async def sample_http_trigger(req: func.HttpRequest) -> func.HttpResponse:
    try:
        dynamics_connetion = Connection()
        entities = [
            ("ecofact_issues", "ecofact_issueid", "ecofact_name"),
            ("ecofact_ecbdimensions", "ecofact_ecbdimensionid", "ecofact_name"),
            ("ecofact_sectorl1s", "ecofact_sectorl1id", "ecofact_name"),
            ("ecofact_timehorizons", "ecofact_timehorizonid", "ecofact_name"),
            ("ecofact_likelihoods", "ecofact_likelihoodid", "ecofact_name"),
            ("ecofact_severities", "ecofact_severityid", "ecofact_name"),
            ("ecofact_riskdrivers", "ecofact_riskdriverid", "ecofact_name"),
            ("ecofact_geographies", "ecofact_geographyid", "ecofact_name"),
            ("ecofact_idiosyncrasies", "ecofact_idiosyncrasyid", "ecofact_name")
        ]
        required_values = dynamics_connetion.load_required_information(entities)
        ...
        #response = await caller.load(text_1, required_values)
        #logging.info(type(response))
        #logging.info(response)
        return func.HttpResponse(
                f"Received Data from Dynamics",
                status_code=200
            )
    except Exception:
        return func.HttpResponse(
